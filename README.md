...
- **Two gestures, two meanings.** In ambient mode a **single click** opens your Jellyfin client (above); over the **artwork** a **double click** brings the active source's own window to the front, so a track playing in a background Safari tab is one gesture away. Focusing only works on sources that advertise it (the YouTube bridge does; Jellyfin doesn't, since the web app is what you'd be looking at anyway). Both gestures ignore repeats for a second, so an accidental double click can't fire twice.
