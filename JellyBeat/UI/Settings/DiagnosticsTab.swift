...
/// Reads the last ~100 `os_log` entries written under JellyBeat's subsystem
    ...
    private var list: some View {
        if let loadError {
            ...
            ContentUnavailableView {
                ...
                Text("JellyBeat writes to os_log under the subsystem software.trypwood.jellybeat. As soon as something happens (a poll, a command, an error), it will show up here.")
                    .padding(.horizontal)
            }
        } else {
            ScrollView {
    ...
    private static func readEntries() async throws -> [LogEntry] {
        try await Task.detached(priority: .userInitiated) {
            ...
            let mapped = entries
                ...
                .filter { $0.subsystem == "software.trypwood.jellybeat" }
                .suffix(100)
                .map { entry in
                    LogEntry(
                        id: UUID(),
