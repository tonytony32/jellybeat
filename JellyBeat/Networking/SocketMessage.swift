...
/// Outgoing WebSocket message from JellyBeat to the Jellyfin server.
///
/// Jellyfin's protocol uses PascalCase keys (`MessageType`, `Data`). The
/// `Data` payload is a string in every outgoing message JellyBeat sends.
...
///
/// Only the message types JellyBeat actually consumes are modelled; every
